...
border-gray-300/20 overflow-hidden transition-[width] duration-300 ${isOpen?"max-md:w-full":"max-md:w-0"}`}>
  ...
  <Link onClick={()=>{scrollTo(0,0);setIsOpen(false)}} to="/">Home</Link>
   <Link onClick={()=>{scrollTo(0,0);setIsOpen(false)}} to="/movies">Movies</Link>
    <Link onClick={()=>{scrollTo(0,0);setIsOpen(false)}} to="/">Therater</Link>
   ...
   <div className='flex items-center gap-8'>
    ...
    {!user?(<button onClick={openSignIn} className='py-1 px-4 sm:px-7 sm:py-2
     btn-base bg-primary hover:bg-primary-dull rounded-full
   transition  font-medium  cursor-pointer'>Login</button>):(
   <UserButton>
